...
public class NumberTriangle {
    ...
    public static NumberTriangle loadTriangle(String fname) throws IOException {
        ...
        NumberTriangle[] previous_triangles = {top};

        String line = br.readLine();
        while (line != null) {
            String[] numbers = line.split(" ");
            NumberTriangle[] triangles = new NumberTriangle[numbers.length];
            for (int i = 0; i < numbers.length; i++) {
                triangles[i] = new NumberTriangle(Integer.parseInt(numbers[i]));
            }
            for (int i = 0; i < previous_triangles.length; i++) {
                previous_triangles[i].left = triangles[i];
                previous_triangles[i].right = triangles[i+1];
            }
